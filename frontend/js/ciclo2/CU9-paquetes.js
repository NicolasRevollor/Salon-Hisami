// =============================================================================
// CU9-paquetes.js — GESTIÓN DE PAQUETES PROMOCIONALES (panel admin)
// Ciclo 2 — Gestión de paquetes de servicios con precio especial
// Un paquete agrupa varios servicios con un precio promocional y fechas de vigencia.
...
// Depende de: main.js (API_BASE, paquetesCache, mostrarToast, formatearFecha)
// =============================================================================

// Carga y muestra todos los paquetes con sus servicios incluidos en la tabla del panel admin
async function cargarPaquetesAdmin() {
    try {
        ...
        data.paquetes.forEach(p => {
            ...
            // Formatear fechas usando formatearFecha (extrae YYYY-MM-DD de timestamps de PostgreSQL)
...
async function abrirModalPaquete(idPaquete = null) {
    ...
    if (idPaquete !== null) {
        ...
        // MODO CREAR: solo limpiar título e id oculto
...
// Cierra el modal de paquetes sin guardar cambios
function cerrarModalPaquete() {
    document.getElementById('modal-paquete').classList.add('seccion-oculta');
}

// Maneja el submit: crea (POST) o edita (PUT) el paquete con los servicios seleccionados.
// Valida nombre, precio y que la fecha de fin no sea anterior a la de inicio.
async function manejarGuardarPaquete(e) {
    ...
    // Recoger los IDs de checkboxes marcados → servicios incluidos en el paquete
...
// Pide confirmación y elimina el paquete.
// Los registros de detalle_paquete se eliminan automáticamente por CASCADE en la FK.
